...
COLORS = ['red', 'yellow', 'DarkOrange1', 'green', 'OliveDrab4', 'DarkSlateGray4', 'cyan', 'blue', 'orchid1', 'maroon4'] #max of 10 classes for now
...
class LabelTool():
    ...
    def loadDir(self, dbg=False):
        ...
        filepath = os.path.realpath(filedialog.askopenfilename(filetypes=[('data', '.jpg .png .mp4 .avi .mov .MOV')]))

        folder_path = os.path.dirname(filepath)
        filename = os.path.basename(filepath)
        filename = os.path.splitext(filename)[0]

        if os.path.splitext(filepath)[1] in {'.mp4','.avi','.mov','.MOV'}:
    ...
    def mouseRelease(self,event):
        ...
        self.mainPanel.itemconfig(self.bboxId, dash=(255,1))
    ...
    def videoProcessing(self, videopath,filename):
        ...
        if os.name == 'nt':
            duration = subprocess.check_output(['ffprobe.exe', '-i', videopath, '-show_entries', 'format=duration', '-v', 'quiet', '-of', 'csv=%s' % ("p=0")])
            fps_check = subprocess.check_output(['ffprobe.exe', '-i', videopath,"-v","0","-select_streams","v","-print_format","flat","-show_entries","stream=r_frame_rate"])
            rate = fps_check.split('=')[1].strip()[1:-1].split('/')
            if len(rate)==1:
                fps =  float(rate[0])
            if len(rate)==2:
        ...
        if float(start) < float(end):
            ...
            if float(fps) <= float(native_fps):
                ...
                if float(end) <= float(native_duration):
                    ...
                    if os.name =='nt':
                        slices = subprocess.check_output(['ffmpeg.exe', '-i', videopath,'-ss',str(start),'-t',str(segment), '-q:v', '2', '-r', str(fps), os.path.join(self.imageDir,filename+"%5d.jpg")])
                    else:
