...
#opens popup and loads selected directory of images
    def loadDir(self, dbg=False):
        ...
        if not os.path.exists(self.outDir):
            ...
            print 'found class list!, loading...\n'+self.outDir
            self.loadlabels()
            i = 0
...
#read existing labels and unlock gui tools
    def loadlabels(self):
        ...
        if os.path.exists(labelfile):
            print "found labelsfile"+ labelfile
            with open(labelfile,"r") as labels:
                i = 0
                for label in labels:
                    print label.rstrip('\n')+str(i)
                    self.classnames[i] = label.rstrip('\n')
                    self.classlist[i].config(state=NORMAL)
                    self.classlist[i].delete(0, END)
                    self.classlist[i].insert(0, label.rstrip('\n'))
...
#defines classes
    def classdefine(self):
        for i in range(0, 10):
            ...
            lbl.insert(0, self.classnames[i])
